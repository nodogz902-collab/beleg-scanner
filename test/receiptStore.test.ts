...
describe('receiptStore', () => {
  ...
  it('listMonths behandelt null-betrag als 0', async () => {
    await saveReceipt(mk('a', '2026-07-01', 1000))
    await saveReceipt(mk('b', '2026-07-02', null))
    const months = await listMonths()
    expect(months[0]).toEqual({ jahr: 2026, monat: 7, count: 2, summe: 1000 })
  })
  it('queryReceipts filtert nach Monat, Lieferant, Tag, Text', async () => {
    ...
    await saveReceipt(mk('c', '2026-08-01', 300, { lieferant: 'Saturn', tags: [], ocrText: '' }))
    expect((await queryReceipts({ jahr: 2026, monat: 7 })).map(r => r.id)).toEqual(['b', 'a'])
  ...
  it('queryReceipts filtert nach kategorie', async () => {
    await saveReceipt(mk('a', '2026-07-01', 100, { kategorie: 'Büro' }))
    await saveReceipt(mk('b', '2026-07-02', 200, { kategorie: 'Reise' }))
    expect((await queryReceipts({ kategorie: 'Büro' })).map(r => r.id)).toEqual(['a'])
    expect((await queryReceipts({ kategorie: 'Reise' })).map(r => r.id)).toEqual(['b'])
  })
})
